...
function page() {
    ...
    const [pickUpLang, setPickUpLang] = useState<number>();
    const [dropCountry, setDropCountry] = useState("");
    const [dropLat, setDropLat] = useState<number>();
    const [dropLang, setDropLang] = useState<number>();
    ...
    const canContinue = !!(vehicle && mobile && pickUp && drop && pickUpLat && pickUpLang && dropLat && dropLang)
        ...
        navigator.geolocation.getCurrentPosition(async ({coords}) => {
            ...
            try {
                ...
                if (data.features.length) {
                    ...
                    setPickUpLang(coords.longitude);
                    ...
                    <div className='p-6 space-y-7'>
                            ...
                            <div className='grid grid-cols-2 gap-2.5'>
                                {
                                    VEHICLES.map((v,i) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${active ? "bg-white" : "bg-zinc-200"}`}>
                                                ...
                                                >
                                                    {
                                                        pickUpSuggestions.map((p,i) => (
                                                            ...
                                                            >
                                                                ...
                                                                <ChevronRight size={13} className='text-zinc-400 flex shrink-0 ml-auto' />
                                                            </motion.div>
                                                        ))
                                                    }
                                                </motion.div>
                        ...
                        >
                            <motion.button
                                ...
                                onClick={() => {
                                    router.push(`/user/search?pickup=${encodeURIComponent(pickUp)}&drop=${encodeURIComponent(drop)}&vehicle=${vehicle}&mobile=${encodeURIComponent(mobile)}&pickuplat=${pickUpLat}&pickuplong=${pickUpLang}&droplat=${dropLat}&droplong=${dropLang}`)
                                }}
                            >
                                <span>
                                    Continue
